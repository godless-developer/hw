...
export default function Home() {
  ...
  const audioScareRef = useRef<HTMLAudioElement | null>(null);
  ...
  // 👻 scare trigger
  const triggerScare = () => {
    if (audioScareRef.current) {
      // start scary.mp3 immediately
      audioScareRef.current.currentTime = 0;
      audioScareRef.current.play().catch(() => {});
    }

    // delay the image by 1s
    setTimeout(() => {
      setShowScare(true);
      ...
      setTimeout(() => {
        ...
        // start looping background scary2.wav
        if (audioLoopRef.current) {
          audioLoopRef.current.loop = true;
          audioLoopRef.current.play().catch(() => {});
        }
      }, 4000); // scare image duration
    }, 500); // 1s delay before showing image
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center md:p-16 p-6 overflow-hidden text-white">
      {/* 🎵 global looping bg audio */}
      <audio ref={audioLoopRef} src="/scary2.wav" preload="auto" loop />

      {/* 🎵 scary.mp3 only for the scare */}
      <audio ref={audioScareRef} src="/scary.mp3" preload="auto" />
      ...
      {showVideo && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black">
          <video
            ...
            onEnded={() => {
              // when video ends, show countdown but keep scary2.wav looping
              setShowVideo(false);
              setShowCount(true);
              localStorage.setItem("hasSeenHalloween", "true");
              if (audioLoopRef.current) {
      ...
      {/* ⏱️ countdown screen */}
      {showCount ? (
        <div className="fixed inset-0 flex flex-col items-center justify-center text-center bg-gradient-to-b from-black via-[#0a0000] to-[#1a0000] text-white">
          ...
          <div className="mt-4 w-40 h-0.5 bg-gradient-to-r from-transparent via-[#ff0000] to-transparent animate-pulse" />
        ...
        <div className="bg-white rounded-2xl w-full max-w-2xl py-6 space-y-6 shadow-2xl z-10 border-y-12 border-blue-900 text-black">
          ...
          <div className="flex flex-col gap-4 px-6">
            ...
            <p className="text-center text-gray-500 text-sm">
              Та энэхүү судалгааг үнэн зөв, шударгаар бөглөнө үү. Хариултууд
              тань бидэнд байгууллагын соёлыг сайжруулахад тусална.
            </p>
          </div>
